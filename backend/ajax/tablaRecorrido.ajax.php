	...
	public function mostrarTabla(){
		...
		if(count($recorrido)== 0){
	 	...
	 	foreach ($recorrido as $key => $value) {
			...
			$acciones = "<div class='btn-group'><button class='btn btn-warning btn-sm editarRecorrido' data-toggle='modal' data-target='#editarRecorrido' idRecorrido='".$value["id"]."'><i class='fas fa-pencil-alt text-white'></i></button><button class='btn btn-danger btn-sm eliminarRecorrido' idRecorrido='".$value["id"]."' imgGrandeRecorrido='".$value["foto_grande"]."' imgPeqRecorrido='".$value["foto_peq"]."'><i class='fas fa-trash-alt'></i></button></div>";	
